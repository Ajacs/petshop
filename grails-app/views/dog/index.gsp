		...
		<div id="list-dog" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="description" title="${message(code: 'dog.description.label', default: 'Description')}" />
					
						<g:sortableColumn property="name" title="${message(code: 'dog.name.label', default: 'Name')}" />
					
						<g:sortableColumn property="registerAt" title="${message(code: 'dog.registerAt.label', default: 'Register At')}" />
				...
				<g:each in="${dogInstanceList}" status="i" var="dogInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${dogInstance.id}">${fieldValue(bean: dogInstance, field: "description")}</g:link></td>
					
						<td>${fieldValue(bean: dogInstance, field: "name")}</td>
					
						<td><g:formatDate date="${dogInstance.registerAt}" /></td>
